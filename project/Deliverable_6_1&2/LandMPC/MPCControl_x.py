...
class MPCControl_x(MPCControl_base):
    """
    Nominal MPC controller for X position subsystem.
    ...
    def _setup_controller(self) -> None:
        """
        Setup nominal MPC for x-position tracking with soft constraints.
        """
        # Cost matrices - higher penalty on beta to keep tilt small
        Q = np.diag([1.0, 100.0, 1.0, 10.0])  # wy, beta, vx, x - beta penalty increased
        R = np.diag([1.0])                     # d2 - increased to reduce aggressive control
        ...
        # Input constraints: -15 <= d2 <= 15 degrees (converted to radians)
        ...
        # Note: State constraints on beta are NOT enforced as hard constraints
        # The cost function penalizes beta deviations from target (which is zero)
    ...
    def get_u(
        ...
        """
        Solve nominal MPC for x-position tracking.
